...
export class BisectService {
  ...
  public async start(item?: any) {
    ...
    try {
      ...
      if (!badCommit) {
        badCommit = await this.pickCommit(
          'Select "Bad" Commit (Bug Exists)',
          'Select the commit/branch where the bug is present'
        );
      }

      if (!badCommit) return; // Cancelled

      const goodCommit = await this.pickCommit(
        'Select "Good" Commit (Bug Absent)',
        'Select a commit/branch where it worked correctly'
      );
      ...
      if (hasChanges) {
        ...
        if (choice === 'Stash Changes & Continue') {
          ...
          vscode.window.showInformationMessage(
            'Changes stashed. Will be restored after bisect reset.'
          );
        } else {
          return;
        }
      ...
      vscode.window.showInformationMessage(
        'Bisect started! Check the current version and mark it as Good or Bad.'
      );
    } catch (e: any) {
  ...
  public async markGood(item?: any) {
    if (this.state !== BisectState.Active) {
      ...
      vscode.window.showWarningMessage(
        'Bisect is not active. Please start a bisect session first.'
      );
      return;
    }
    try {
  ...
  public async getLog(): Promise<
    { status: 'bad' | 'good' | 'skip'; hash: string; subject?: string }[]
  > {
    const git = this.gitService.executor;
  ...
  public showMenu() {
    const items = [
      {
        label: '$(check) Mark Good',
        description: 'Current version works correctly',
        action: () => this.markGood(),
      },
      {
        label: '$(x) Mark Bad',
        description: 'Current version has the bug',
        action: () => this.markBad(),
      },
      {
        label: '$(debug-step-over) Skip',
        description: 'Cannot test this commit',
        action: () => this.skip(),
      },
      {
        label: '$(stop) Stop Bisect',
